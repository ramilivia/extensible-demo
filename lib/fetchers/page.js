...
import { isPreviewEnabled, getPreviewStatus } from '@/lib/preview'

export async function getPageData(slug = 'home', searchParams = {}, options = {}) {
  const { throwOnNotFound = false, locale = 'en' } = options
  
  try {
    // Use custom preview system
    const previewStatus = getPreviewStatus()
    const previewEnabled = previewStatus.isEnabled
    console.log('Preview mode - custom preview:', previewStatus)
