...
export async function getPageData(slug = 'home', searchParams = {}, options = {}) {
  ...
  try {
    ...
    if (!pageResult) {
      ...
      // Return fallback data for homepage
      if (slug === 'home') {
        return {
          page: { blocks: [] },
          siteConfiguration: {
...
export async function getHomepageData() {
  return getPageData('home', {}, { throwOnNotFound: false })
}

export async function getSlugPageData(slug, searchParams = {}) {
  return getPageData(slug, searchParams, { throwOnNotFound: true })
}
